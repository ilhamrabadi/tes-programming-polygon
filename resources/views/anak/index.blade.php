  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      <form action="{{ route('transactions.delete') }}" method="POST">
        @csrf
        <h5><strong>Yakin ingin menghapus data ini?</strong></h5>
...
{{-- Start Confirm Record Modal --}}
<div id="confirm_record" class="modal fade" tabindex="-1" role="dialog"
aria-labelledby="confirm_recordLabel" aria-hidden="true">
<div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-header modal-colored-header bg-primary">
      <h4 class="modal-title" id="confirm_recordLabel">Konfirmasi Data
      </h4>
      <button type="button" class="close" data-dismiss="modal"
      aria-hidden="true">×</button>
    </div>
    <div class="modal-body">
      <form action="{{ route('transactions.confirm') }}" method="POST">
        @csrf
        @method("PUT")
        <h5><strong>Yakin ingin konfirmasi data ini?</strong></h5>
        <input type="hidden" name="transaction_id" id="transaction_id" value="">
        </div>
        <div class="modal-footer">
          <button type="submit" class='btn btn-success btn-sm' id=''><i class="fas fa-save"></i> Konfirmasi</button>
          <button type="button" class="btn btn-light" data-dismiss="modal">Batal</button>
        </div>
      </form>
    </div><!-- /.modal-content -->
...
@if (session('record-confirm-successful'))
<div class="alert alert-success" role="alert">
  <strong>Sukses - </strong> {{ session('record-confirm-successful') }}
</div>  
@endif
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($t->status == "1") 
                            @else
                            <button class="btn btn-sm btn-success OpenConfirmModal" data-id="{{ $t->id }}" data-toggle="modal" data-target="#confirm_record"><i class="fas fa-check"></i> Konfirmasi</button>
                            @endif
